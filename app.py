...
if menu == "customers":
    ...
    with st.form("add_customer"):
        ...
        if submitted:
            cursor = conn.cursor()
            cursor.execute("insert into customer (customer_id, name, phone, email, license) values (?, ?, ?, ?, ?)",
                           (cid, name, phone, email, license_num))
            conn.commit()
    ...
    with st.form("add_car"):
        ...
        if submitted:
            ...
            cursor.execute("""
                ...
                values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (car_id, model, brand, year, color, plate_id, price_per_day, active, rented, out_of_service, office_id))
            conn.commit()
    ...
    if brand_search:
        query = "select * from car where brand like ? and active = 'yes' and rented = 'no'"
        df_search = pd.read_sql(query, conn, params=[f"%{brand_search}%"])
    ...
    with st.form("update_car"):
        ...
        if update_submitted:
            cursor = conn.cursor()
            cursor.execute("update car set rented = ?, active = ? where car_id = ?", (new_rented, new_active, car_id_update))
            conn.commit()
    ...
    with st.form("add_reservation"):
        ...
        if submitted:
            ...
            cursor.execute("""
                ...
                values (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (rid, cid, car_id, pickup, return_office, start, end, total, status))
            conn.commit()
